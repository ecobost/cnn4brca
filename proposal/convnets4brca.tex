...
As explained in Section~\ref{subsec:BreastCancer} there are two main signs of breast cancer: breast masses and clustered microcalcifications, however, their presence does not necessarily imply cancer as there could be benign masses as well as benign microcalcifications. We refer as detection to the task of classifying a lesion as present or not in the image no matter its malignancy, for instance, classifying an image patch as either clustered microcalcifications or normal tissue, while we talk of diagnosis when classifying a lesion as either benign or malignant~\footnote{If the feature is not actually present on the image it is considered benign.}.
