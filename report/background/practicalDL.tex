...
% I could use ADAM(http://arxiv.org/pdf/1412.6980v8.pdf), too. No need to learn any more parameters, probably not need learning rate decay. https://www.youtube.com/watch?v=hd_KFJ5ktUc
	...
	%Inverted dropout may be better
...
% add batch normalization?. see https://youtu.be/gYpoJMlgyXA min 55, lots of details. 14 times fewer training steps.
...
\begin{itemize}
	...
	\item During training, the loss function evaluated on training examples decreases monotonically~\footnote{Because of momentum and regularization it could increase slightly before decreasing.}. If not, check for implementation errors, reduce the learning rate or augment momentum~\cite{Karpathy2015}.
...
The research community has been moving towards replacing pooling layers and using convolutional filters as pooling operations. We offer a couple of guidelines for implementing all convolutional networks.  
\begin{itemize}
	\item Change each pooling layer with a convolutional layer with as many feature maps as the previous layer and filter size $2\times 2$ with stride $2$ for normal pooling or $3\times 3$ with stride $2$ for overlapping pooling~\cite{Springenberg2014}. 
