...
\begin{figure}[h]
	\centering
	\includegraphics[width = 0.38\textwidth]{plots/convLayer.jpeg}
...
\begin{figure}[h]
	\centering
	\includegraphics[width = 0.9\textwidth]{plots/convNetExample.jpeg}
...
Lately, simpler convolutional network architectures have emerged. The All Convolutional Net~\cite{Springenberg2014} is a network formed solely by convolutional layers: we replace pooling layers by convolutional layers with larger strides. This greatly increases the number of parameters so it is inappropiate for small data sets. \emph{Transfer learning} is a related trend where we train a convolutional network on data from a specific domain and later reuse it to extract image features in a different domain or as an initialized network to fine-tune with the new data.
