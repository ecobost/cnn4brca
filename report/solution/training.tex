...
%compute a mask beforehand with all zeros iu the background and all 1s in the input and do a dot product before the loss function, that way the prediction is always gonna be zero for balck classes which won't count in the loss nor produce gradients.Maybe at the same time you zero-mean the image
